...
module clk_prescaler
  (
    ...
    output reg clk_1us = 0     // OD, STD mode fsm clock
  );

  wire      clk_prescaled;         // prescaled clock output
  reg       clk_div           = 0; // divided clk for hdrive
  reg       en_div            = 0; // enable use of divided clk for hdrive
  reg       clk_prescaled_reg = 0;
  reg [6:0] div_cnt           = 0;
  reg [2:0] ClkPrescale       = 0;
  ...
  always @(posedge rst_clk or posedge CLK)
    if (rst_clk)
      ...
      case(ClkPrescale)
        s0: ClkPrescale <= s1;
        s1: ClkPrescale <= s2;
        s2: ClkPrescale <= (pre_0 && !pre_1) ? s0 : s3;
        s3: ClkPrescale <= s4;
        s4: ClkPrescale <= (!pre_0 && pre_1) ? s0 : s5;
        s5: ClkPrescale <= s6;
        s6: ClkPrescale <= s0;
  ...
  always @(posedge MR or posedge CLK)
    if (MR)
      ...
      clk_prescaled_reg <= (!ClkPrescale[0] && !ClkPrescale[1] && !ClkPrescale[2]);
